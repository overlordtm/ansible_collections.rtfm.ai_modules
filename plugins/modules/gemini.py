    ...
    if safety_settings_input:
        ...
        for key, value in safety_settings_input.items():
            if not isinstance(key, str) or not isinstance(value, str):
                module.fail_json(
                    msg=f"Invalid safety_settings format. Both category ('{key}') and threshold ('{value}') must be strings."
                )
            try:
        ...
        for rating in prompt_feedback_obj.safety_ratings:
            ...
            if hasattr(rating, "to_dict"):
                try:
                    ...
                    module.warn(
                        f"Failed to convert SafetyRating to dict using .to_dict(): {e}"
...
def run_module():
    ...
    if not HAS_GEMINI_LIB:
        module.fail_json(
            msg="The 'google-generativeai' Python library is required. Please install it: pip install google-generativeai"
        )
    ...
    while attempts <= retry_attempts:
        ...
        try:
            ...
            if response_candidates:
                ...
                if not content_blocked:
                    ...
                    try:
                        ...
                        candidate_errors.append(
                            f"Could not extract text from candidate (potential content issue): {str(ve)}"
                        )
                    except Exception as e:
            ...
            module.fail_json(
                msg=f"Gemini API Invalid Argument: {str(e)}. Check model name, parameters, and safety settings.",
                exception=traceback.format_exc(),
            )
